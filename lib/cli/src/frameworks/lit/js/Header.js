...
export class StorybookHeader extends LitElement {
  static get styles() {
    return css`
      .wrapper {
    ...
    return html`
      <header>
        <div class="wrapper">
          ...
          <div>
            ${this.logInOutButton()} 
              <sb-button size="small" @sb-button:click="${() =>
                this.dispatchCustomEvent('createAccount')}"
                         label="Sign up"
              </sb-button>
          </div>
        </div>
...
customElements.define('sb-header', StorybookHeader);
